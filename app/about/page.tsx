...
export default function AboutPage() {
  return (
    <>
      {/* HERO */}
      <section style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', padding: '100px 56px', borderBottom: '1px solid var(--border)', position: 'relative', overflow: 'hidden', background: 'var(--bg)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: 700 }}>
          ...
          <h1 style={{ fontSize: 'clamp(56px,7vw,110px)', fontWeight: 900, lineHeight: 0.9, textTransform: 'uppercase', letterSpacing: '-2px', marginBottom: 28, color: 'var(--text)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', borderBottom: '1px solid var(--border)' }} className="stats-grid">
        {[['100%','Authentic','Every pair is genuine. No replicas ever.'],['2K+','Happy Customers','Karachi to Islamabad and beyond.'],['500+','Styles Available','For every occasion and every man.'],['3–5','Day Delivery','Nationwide. Same-day dispatch in KHI.']].map(([num,label,desc]) => (
          <div key={label} style={{ padding: '48px 28px', borderRight: '1px solid var(--border)', textAlign: 'center', transition: 'background 0.15s' }}
            ...
            <div style={{ fontSize: 52, fontWeight: 900, color: 'var(--gold)', lineHeight: 1, marginBottom: 10 }}>{num}</div>
      ...
      <section style={{ padding: '56px 32px', background: 'var(--bg)' }}>
        <div style={{ marginBottom: 40 }}>
          ...
          <div style={{ fontSize: 52, fontWeight: 900, color: 'var(--text)', lineHeight: 1, textTransform: 'uppercase', letterSpacing: '-1px' }}>OUR VALUES</div>
        </div>
      ...
      <Footer />
      <style>{`.stats-grid>div:last-child{border-right:none!important} @media(max-width:768px){.stats-grid,.val-grid{grid-template-columns:1fr 1fr!important}}`}</style>
    </>
  )
}
